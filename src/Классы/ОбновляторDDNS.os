...
&Деталька(Значение = "DDNS.Verbose", ЗначениеПоУмолчанию = False) 
Перем РасширеннаяИнформация Экспорт; // Управление запуском обновления
...
Перем МаксимальноеКоличествоСтрокОтвета; // Максимальное количество строк ответа
#КонецОбласти
...
Процедура УстановитьПараметрыАдреса() Экспорт 
	URL = СтрШаблон("https://www.duckdns.org/update?domains=%1&token=%2&verbose=%3", Домен, Токен, РасширеннаяИнформация);
...
Процедура РасширеннаяИнформация(ОтветСервера)
	Если РасширеннаяИнформация И ОтветСервера.Количество() = МаксимальноеКоличествоСтрокОтвета Тогда
...
ИндексIP = 1;
ИндексИзменения = 3;
МаксимальноеКоличествоСтрокОтвета = 4;
#КонецОбласти